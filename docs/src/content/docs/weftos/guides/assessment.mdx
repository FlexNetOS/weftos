...
The assessment produces findings in these categories:

| Category | Severity | What it checks |
|----------|----------|----------------|
| `complexity` | medium | Files exceeding 500 lines |
| `technical-debt` | info | TODO/FIXME comments in code |

Future categories (planned):
- `coupling` — unexpected file co-changes
- `coverage` — percentage of code indexed in knowledge graph
- `dependency` — new, removed, or upgraded dependencies
- `regression` — complexity increases vs. previous assessment
