...
Ingest a local path or URL into the knowledge graph. Local paths run the extraction pipeline (tree-sitter AST + call graph inference). URLs are fetched, classified, and saved as annotated markdown for re-extraction.
...
Export the knowledge graph to a file or directory.
