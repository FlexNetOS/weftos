---
title: Installation
description: Install WeftOS from pre-built binaries, Homebrew, Docker, cargo, or npm.
---

# WeftOS Installation Guide

## Quick Install (recommended)

### Shell installer (Linux / macOS)

```bash
curl -fsSL https://github.com/FlexNetOS/weftos/releases/latest/download/clawft-cli-installer.sh | sh
```

### Homebrew (macOS / Linux)

```bash
brew install FlexNetOS/tap/clawft-cli
```

### Download binary manually

Pre-built binaries for all platforms are available on the [GitHub Releases](https://github.com/FlexNetOS/weftos/releases) page.

| Platform | Binary |
|----------|--------|
| Linux x86_64 | `clawft-cli-x86_64-unknown-linux-gnu.tar.gz` |
| Linux ARM64 | `clawft-cli-aarch64-unknown-linux-gnu.tar.gz` |
| macOS Intel | `clawft-cli-x86_64-apple-darwin.tar.gz` |
| macOS Apple Silicon | `clawft-cli-aarch64-apple-darwin.tar.gz` |
| Windows x86_64 | `clawft-cli-x86_64-pc-windows-msvc.zip` |

### PowerShell installer (Windows)

```powershell
irm https://github.com/FlexNetOS/weftos/releases/latest/download/clawft-cli-installer.ps1 | iex
```

## Verify

```bash
weft --version    # Should print: weft 0.1.x
weft status       # Show configuration and defaults
weft tools list   # Show built-in tools
```

## Other install methods

### Docker

```bash
docker pull ghcr.io/flexnetos/weftos:latest
docker run --rm ghcr.io/flexnetos/weftos weft --version
```

For persistent use:

```bash
docker run -d --name weftos \
  -v weftos-data:/home/nonroot/.clawft \
  -p 8080:8080 \
  ghcr.io/flexnetos/weftos
```

See the [Docker deployment guide](/weftos/guides/deployment-docker) for production setup.

### cargo install (builds from source)

Requires Rust 1.93+:

```bash
cargo install weftos
```

This installs the `weftos` daemon binary. For the `weft` CLI:

```bash
# Clone and build (weft is not published as a standalone crate)
git clone https://github.com/FlexNetOS/weftos.git
cd weftos
cargo install --path crates/clawft-cli
```

### cargo binstall (pre-built binary via cargo)

```bash
cargo binstall clawft-cli
```

### npm (WASM module for browser)

```bash
npm install @weftos/core
```

This gives you the WeftOS WASM runtime for browser-based agent applications.

### Rust library (add to your project)

```toml
[dependencies]
weftos = "0.1"

# Or individual framework crates:
clawft-core = "0.1"
clawft-kernel = "0.1"
clawft-types = "0.1"
clawft-plugin = "0.1"
```

All crates are on [crates.io](https://crates.io/crates/weftos).

## Three binaries

WeftOS ships three binaries:

| Binary | Purpose | Install via |
|--------|---------|-------------|
| `weft` | Agent CLI (primary user interface) | `clawft-cli` package |
| `weaver` | Kernel operator CLI (daemon management) | `clawft-weave` package |
| `weftos` | Kernel daemon | `weftos` package |

## From source (development)

```bash
git clone https://github.com/FlexNetOS/weftos.git
cd weftos

# Build all binaries in release mode
scripts/build.sh native

# Or debug mode (faster compilation)
scripts/build.sh native-debug

# Full feature set
scripts/build.sh native --features ecc,exochain,os-patterns,mesh,cluster,onnx-embeddings,wasm-sandbox
```

See the [feature flags guide](/weftos/guides/feature-flags) for the complete feature reference.

### ONNX embeddings (optional)

```bash
scripts/download-model.sh
scripts/build.sh native --features onnx-embeddings
```

### Compile-time checks

```bash
scripts/build.sh gate   # 11 checks: build, test, clippy, etc.
```

## Troubleshooting

| Problem | Fix |
|---------|-----|
| `weft: command not found` after install | Add `~/.cargo/bin` to your PATH |
| Homebrew formula not found | `brew tap FlexNetOS/tap` first |
| Docker permission denied | `docker login ghcr.io` or check GHCR auth |
| `rustup` reports Rust < 1.93 | `rustup update stable` |
| ONNX model not found | Run `scripts/download-model.sh` first |

## Next steps

- [Quickstart](/weftos/getting-started/quickstart) -- get a kernel running in 2 minutes
- [Feature Flags](/weftos/guides/feature-flags) -- compile-time feature flags
- [Configuration](/weftos/guides/configuration) -- runtime configuration
- [First Project](/weftos/getting-started/first-project) -- create your first WeftOS project
