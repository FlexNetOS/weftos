...
- **`weaver graphify rebuild` now functional**: Runs the full detect -> extract -> build -> cluster -> analyze -> export -> report pipeline, producing `graphify-out/graph.json` and `graphify-out/GRAPH_REPORT.md`
