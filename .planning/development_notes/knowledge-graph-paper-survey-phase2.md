...
The sonobuoy-specific analysis of K-STEMIT has been **moved** to its own
project folder alongside additional sonar / fish-ID / marine-bioacoustics
research:

➡ `.planning/sonobuoy/k-stemit-sonobuoy-mapping.md`

Summary: K-STEMIT's dual-branch GraphSAGE + gated-temporal-convolution
architecture maps onto a unified detect → bearing-estimate → species-ID
pipeline for a distributed hydrophone array. Priority **P0** for the
sonobuoy project. See the extracted doc for the full radar-to-acoustic
signal-processing mapping, physics-informed node features, and learned
beamforming argument.
